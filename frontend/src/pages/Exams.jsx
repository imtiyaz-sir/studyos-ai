...
export default function Exams() {
  ...
  const confirmDelete = async () => {
    setDeleting(true);
    try {
      await api.del(`/api/exams/${confirmExam.id}`);
      toast(`"${confirmExam.title}" deleted`);
      setConfirmExam(null);
      load();
    } finally {
      setDeleting(false);
  ...
  return (
    <>
                  ...
                  {upcoming.map((e) => {
                    ...
                    return (
                      <div key={e.id} className="card p-4 flex items-center gap-3.5">
                        ...
                        <button onClick={() => setConfirmExam(e)} className="btn-icon hover:text-rose-500 shrink-0">
                          <Trash2 size={14} />
                        </button>
                      </div>
                    );
      ...
      <ConfirmDialog
        ...
        loading={deleting}
        title="Delete Exam?"
        message={`This will permanently remove "${confirmExam?.title || ""}". This action cannot be undone.`}
      />
    </>
  );
}
