...
export default function CalendarPage() {
  ...
  const confirmDelete = async () => {
    setDeleting(true);
    try {
      await api.del(`/api/calendar/${confirmEvent.id}`);
      toast(`"${confirmEvent.title}" deleted`);
  ...
  return (
    <>
      ...
      <ConfirmDialog
        ...
        loading={deleting}
        title="Delete Event?"
        message={`This will permanently remove "${confirmEvent?.title || ""}" from your calendar. This action cannot be undone.`}
      />
    </>
  );
}
