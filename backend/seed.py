...
def run():
    ...
    uid = execute(
        "INSERT INTO users (name, email, password_hash, xp, coins, level, current_streak, longest_streak) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
    ...
    for name, (color, units) in subjects_data.items():
        ...
        for u_idx, (unit_name, topics) in enumerate(units):
            ...
            for t_idx, topic_name in enumerate(topics):
                ...
                if status == "completed":
                    running = date.today()
                    for i, gap in enumerate([1, 3, 7, 16, 35], start=1):
                        running = running + timedelta(days=gap)
                        execute(
                            "INSERT INTO revisions (topic_id, revision_number, scheduled_date, status) VALUES (?, ?, ?, ?)",
                            (topic_id, i, running.isoformat(), "pending" if i > 1 else "done"),
                        )
                        if i == 1:
                            execute(
                                "UPDATE revisions SET completed_date = ?, memory_strength = 80, confidence_level = 80 WHERE topic_id = ? AND revision_number = 1",
    ...
    for name, icon in [("Daily Study (2h+)", "book-open"), ("Morning Revision", "brain"),
        ...
        execute("INSERT OR IGNORE INTO habit_logs (habit_id, log_date, completed) VALUES (?, ?, 1)", (hid, date.today().isoformat()))
        execute("INSERT OR IGNORE INTO habit_logs (habit_id, log_date, completed) VALUES (?, ?, 1)", (hid, (date.today() - timedelta(days=1)).isoformat()))
        for d in range(2, 7):
            if random.random() > 0.3:
                log_date = (date.today() - timedelta(days=d)).isoformat()
                execute("INSERT OR IGNORE INTO habit_logs (habit_id, log_date, completed) VALUES (?, ?, 1)", (hid, log_date))
    ...
    for d in range(14):
        ...
        execute(
            """INSERT OR IGNORE INTO daily_stats (user_id, stat_date, study_minutes, tasks_completed, revisions_done, practice_sessions, productivity_score)
               VALUES (?, ?, ?, ?, ?, ?, ?)""",
