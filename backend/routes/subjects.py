...
def list_subjects():
    uid = current_user_id()
    subjects = query("SELECT * FROM subjects WHERE user_id = ? ORDER BY created_at", (uid,))
    for s in subjects:
        units = query("SELECT id FROM units WHERE subject_id = ?", (s["id"],))
        unit_ids = [u["id"] for u in units] or [-1]
        placeholders = ",".join("?" * len(unit_ids))
        topics = query(f"SELECT status FROM topics WHERE unit_id IN ({placeholders})", unit_ids)
        s["unit_count"] = len(units)
        s["topic_count"] = len(topics)
        s["completion_pct"] = _topic_completion(topics)
...
def get_subject(subject_id):
    ...
    for u in units:
        flat_topics = query("SELECT * FROM topics WHERE unit_id = ? ORDER BY order_index", (u["id"],))
